...
from Hermione import dispatcher, telethn
from Hermione.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
from Hermione.modules.helper_funcs.chat_status import dev_plus, sudo_plus
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("Hermione.modules." + text)
    except:
...
def unload(update: Update, context: CallbackContext):
    ...
    if hasattr(imported_module, "__basic_cmds__") and imported_module.__basic_cmds__:
        BASICCMDS.pop(imported_module.__mod_name__.lower())
    
    if hasattr(imported_module, "__funtools__") and imported_module.__funtools__:
        FUNTOOLS.pop(imported_module.__mod_name__.lower())
     
    if hasattr(imported_module, "__advtools__") and imported_module.__advtools__:
        ADVTOOLS.pop(imported_module.__mod_name__.lower())
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Hermione.modules.", 1)[1]
