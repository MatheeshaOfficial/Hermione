...
from Hermione.db.mongo_helpers.filterdb import (
    delete_filter,
    get_filter,
    get_filters_names,
    save_filter,
)
from Hermione.function.pluginhelpers import member_permissions
from Hermione import pbot as app
...
async def save_filters(_, message):
    if len(message.command) < 2 or not message.reply_to_message:
        await message.reply_text(
            "Usage:\nReply to a text or sticker with /addfilter <textfilter name> to save it. \n\n NOTE: **TRY OUR NEW FILTER SYSTEM WITH /addfilter**"
        )
