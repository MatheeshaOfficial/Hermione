...
import urllib
import validators
from Hermione.function.pluginhelpers import edit_or_reply, get_text, humanbytes
...
async def download_img(url):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status == 200:
                file_path = f"Webshot_Hermione.png"
...
@pbot.on_message(filters.command("webshot") & ~filters.edited & ~filters.bot)
...
__mod_name__ = "ᴡᴇʙꜱꜱ📸"
HELPSTRINGS = "/webshot [URL] - Take A Screenshot Of A Webpage"
